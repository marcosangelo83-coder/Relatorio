...
function notifyStateChange() {
  renderKPIs();
  renderUnitCategoryTable(); // Atualiza a nova Tabela por Unidade e Categoria
  renderSyntheticTable();
  renderAnalyticalTable();
}

/* === INÍCIO: TABELA DE UNIDADES E CATEGORIAS ECONÔMICAS === */
function renderUnitCategoryTable() {
  ...
  // Agrupamento por Unidade e Categoria Econômica
  const grouped = {};
  let grandTotal = 0;
  ...
  data.forEach(item => {
    ...
    if (!grouped[unidade][categoria]) {
      grouped[unidade][categoria] = 0;
    }

    grouped[unidade][categoria] += valor;
    grandTotal += valor;
  });

  // Renderização das linhas organizadas por Unidade e Categoria
  Object.keys(grouped).sort().forEach(unidade => {
    const categorias = grouped[unidade];
