...
function notifyStateChange() {
  renderKPIs();
  renderUnitCategoryTable(); // Executa a renderização da tabela de Unidades e Categorias
  renderSyntheticTable();
  renderAnalyticalTable();
}

/* === TABELA DE UNIDADES E CATEGORIAS ECONÔMICAS === */
function renderUnitCategoryTable() {
  const tbody = document.getElementById('unitCategoryTableBody');
  const tfoot = document.getElementById('unitCategoryTableFooter');
  ...
  if (!data || data.length === 0) {
    tbody.innerHTML = `<tr><td colspan="3" class="px-4 py-8 text-center text-gray-400">Nenhum dado disponível para os filtros selecionados.</td></tr>`;
    return;
  }
  ...
  data.forEach(item => {
    const unidade = (item['Unidade'] && item['Unidade'].toString().trim()) ? item['Unidade'].toString().trim() : '(Não Informado)';
    ...
    const valor = item['Valor Estimado'] || 0;

    if (!grouped[unidade]) grouped[unidade] = { categorias: {}, totalUnidade: 0 };
    if (!grouped[unidade].categorias[categoria]) grouped[unidade].categorias[categoria] = 0;

    grouped[unidade].categorias[categoria] += valor;
    grouped[unidade].totalUnidade += valor;
    grandTotal += valor;
  });

  Object.keys(grouped).sort().forEach(unidade => {
    const { categorias, totalUnidade } = grouped[unidade];

    Object.keys(categorias).sort().forEach(categoria => {
      const valorTotal = categorias[categoria];
      const tr = document.createElement('tr');
      tr.className = "hover:bg-gray-50 border-b border-gray-100 transition-colors text-xs";
      ...
      tr.innerHTML = `
        <td class="px-4 py-2.5 font-medium text-gray-800">${unidade}</td>
        <td class="px-4 py-2.5">
          <span class="inline-block px-2 py-0.5 text-xs rounded font-medium ${badgeClass}">
            ${categoria}
          </span>
        </td>
        <td class="px-4 py-2.5 text-right font-semibold text-gray-900">${formatBRL(valorTotal)}</td>
      `;
    ...
    trSub.className = "bg-slate-100/70 font-semibold border-b border-slate-200 text-xs text-slate-800";
    trSub.innerHTML = `
      <td colspan="2" class="px-4 py-2 text-right">Subtotal ${unidade}:</td>
      <td class="px-4 py-2 text-right text-slate-900 font-bold">${formatBRL(totalUnidade)}</td>
    `;
  ...
  tfoot.innerHTML = `
    <tr>
      <td colspan="2" class="px-4 py-3 text-right font-bold text-gray-800">TOTAL GERAL:</td>
